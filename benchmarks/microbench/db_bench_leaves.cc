...
static leaves::Slice TrimSpace(leaves::Slice s) {
  int start = 0;
  ...
  return leaves::Slice(s.data() + start, limit - start);
}
...
class Benchmark {
 private:
  ...
  void PrintEnvironment() {
    fprintf(stderr, "Leaves:     version %s\n", leaves::version().data());
    ...
    if (cpuinfo != NULL) {
      ...
      while (fgets(line, sizeof(line), cpuinfo) != NULL) {
        ...
        leaves::Slice key = TrimSpace(leaves::Slice(line, sep - 1 - line));
        leaves::Slice val = TrimSpace(leaves::Slice(sep + 1));
        if (key == Slice("model name")) {
          ++num_cpus;
          cpu_type = val.string();
        } else if (key == Slice("cache size")) {
          cache_size = val.string();
        }
      }
      fclose(cpuinfo);

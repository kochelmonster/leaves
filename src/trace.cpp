...
INLINE bool Transition::follow_link(Trace& trace, const offset_ptr* link) {
  ...
  if (link->pool_id() == LEAF_BLOCK) {
    ...
    leaf.leaf()->leaf(*link)->find(trace);
    return false;
  }
...
struct Inserter {
  ...
  void complete(Transition& back);

  // alloc size in branch and addtionally create the leafs for _value and
  // _old_leaf_link
...
inline void Inserter::_start() {
  ...
  if (_back->branch->has_trie()) return add_to_trie(ioffset);

  extend_compressed();
}

inline bool Inserter::check_stack() {
  for (size_t i = 0; i < _trace.stack.size - 1; i++) {
    Transition& t = _trace.stack.data[i];
    assert(t.success == false);
    assert(t.found_leaf == nullptr);
  }
  return true;
}
...
inline void Inserter::first() {
  ...
  Slice prefix = _trace.rest_key;
  _trace.advance_key(prefix.size());
  ...
  _back->keypos = 0;
  if (_back->prefix) {
    _back->branch->add_compressed(prefix.data(), _back->prefix);
  }
...
inline void Inserter::add_to_array(bsize_t ioffset) {
  ...
  if (n->size < ArrayBranch::COUNT) {
    ...
    _back->index = n->size++;
    _back->olink = _back->branch->olink(&n->links[_back->index]);
    n->keys[_back->index] = c;
    n->links[_back->index] = _new_leaf_link;
  } else {
    ...
    _back->index = tmp.index(c);
    _back->olink = _back->branch->olink(&n->links[_back->index]);

    tmp.links[_back->index] = _new_leaf_link;
...
inline void Inserter::add_to_trie(bsize_t ioffset) {
  ...
  assert(_back->index == _trace.rest_key[0]);

  TrieBranch* n = (TrieBranch*)&_back->branch->data[ioffset];
  assert(n->bits[_back->tindex.idx] & (1 << _back->tindex.bit) == 0);
...
inline void Inserter::extend_leaf() {
  ...
  *_back->plink() = _new_leaf_link;
  ...
  parent->found_leaf = nullptr;
  *parent->plink() = _back->branch->offset;
}
...
inline bsize_t Inserter::alloc_branch(bsize_t space) {
  ...
  if (_back->branch->freespace() < space) {
    block_ptr org = _back->branch;
    _back->branch =
        _trace.storage.alloc_block_by_pool(_back->branch->offset.pool_id());
...
INLINE void Stack::push(block_ptr block) {

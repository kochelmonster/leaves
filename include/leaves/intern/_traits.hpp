...
struct SmartPointer {
  ...
  struct ptr {
    static constexpr NodeTypes type = TRIE;
    ptr(_AreaPtr* first = nullptr) : _iref(first) {
      if (_iref) _iref->ref.fetch_add(1, std::memory_order_acq_rel);
    }
    ...
    ~ptr() {
      auto* tmp = _iref;
      if (tmp && tmp->ref.fetch_sub(1, std::memory_order_acq_rel) == 1) {
        // _AreaPtr was placement-new'ed into a raw allocation; delete the raw
        // block
  ...
  struct Pointer : public ptr {
    ...
    Pointer(void* src = nullptr)
        : ptr(reinterpret_cast<typename SmartPointer<BlockHeader>::_AreaPtr*>(
              src)) {}
    Pointer(const ptr& src) : ptr(src) {}
    const ptr& operator=(const ptr& src) {
      if (ptr::_iref) ptr::_iref->ref.fetch_sub(1, std::memory_order_acq_rel);
      ptr::_iref = src._iref;
      if (ptr::_iref) ptr::_iref->ref.fetch_add(1, std::memory_order_acq_rel);
      return src;
    }

...
struct SimplePointer {
  ...
  SimplePointer(std::nullptr_t) : p(nullptr) {}  // Explicit nullptr_t constructor
  ...
  template <typename, NodeTypes> friend struct SimplePointer;
...
struct SmartPointer {
  ...
  SmartPointer(std::nullptr_t) : _iref(nullptr), _offset(0) {}  // Explicit nullptr_t constructor
  SmartPointer(const SmartPointer& src) : _iref(src._iref), _offset(src._offset) {
    if (_iref) _iref->inc_ref();
  }
  SmartPointer(SmartPointer&& other) noexcept : _iref(other._iref), _offset(other._offset) {
    other._iref = nullptr;
    other._offset = 0;
  }
  ...
  template <typename U, NodeTypes u>
  SmartPointer(const SmartPointer<U, u>& src) : _iref(src._iref), _offset(src._offset) {
    if (_iref) _iref->inc_ref();
  }
  ...
  operator const T*() const {
    return (const T*)((const char*)*this);
  }
  ...
  template <typename, NodeTypes> friend struct SmartPointer;

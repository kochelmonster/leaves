...
// All methods are identity / allow-all / no-op,
...
//                          on_prepare_commit, before/on_commit (no CursorContext)
...
struct DefaultAspect {
  ...
  constexpr bool before_start_transaction(DB&, TransactionOrigin, Ctx&) { return true; }

  template <typename DB, typename Ctx>
  constexpr void on_start_transaction(DB&, tid_t, TransactionOrigin, Ctx&) {}

  template <typename DB, typename Ctx>
  constexpr bool before_rollback(DB&, tid_t, TransactionOrigin, Ctx&) { return true; }

  template <typename DB, typename Ctx>
  constexpr void on_rollback(DB&, tid_t, TransactionOrigin, Ctx&) {}

  template <typename DB, typename Ctx>
  constexpr bool before_commit(DB&, TransactionOrigin, Ctx&) { return true; }

  /// Called after a successful cursor-level transaction commit.
  template <typename DB, typename Ctx>
  constexpr void on_commit(DB&, TransactionOrigin, Ctx&) {}
  ...
  template <typename DB>
  constexpr bool before_defrag(DB&) { return true; }

  template <typename DB>
  constexpr void on_defrag(DB&) {}

  template <typename DB>
  constexpr bool before_reset(DB&) { return true; }

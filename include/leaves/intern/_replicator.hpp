...
struct _RDB {
  ...
  /*
    Build the query trie to select the branches to send to the peer
  */
  template <typename Resolver>
  void build_query(offset_t me, offset_t peer, Resolver& peer_resolver) {
    assert(me.type() == TRIE);
    assert(peer.type() == TRIE);

    trie_ptr nme = resolve(me);
    trie_ptr npeer = peer_resolver.resolve(peer);

    if (!memcmp(nme->hash, peer->hash, sizeof(peer->hash))) return;

    Slice pme(nme->compressed(), nme->len()),
        ppeer(npeer->compressed(), npeer->len());

    int cmp;
    while (true) {
      auto prefix =
          get_prefix(pme.data(), ppeer.data(), pme.size(), ppeer.size(), cmp);
      if (prefix == pme.size() && prefix < ppeer.size()) {
        if (!nme->isset(ppeer[prefix])) return;

        trie_ptr n = ndb->alloc(nme->size());
        *parent = ndb->resolve(n);
        memcpy(&*n, &*nme, nme->size());
        memset(n->array(), 0, n->array_size());
        me = *nme->offset(ppeer[prefix]) nme = resolve(me);
        pme = Slice(nme->compressed(), nme->len()) ppeer.iadvance(prefix);
        continue;
      }

      if (prefix == ppeer.size() && prefix < pme.size()) {
        if (!npeer->isset(pme[prefix])) return;
        peer = *npeer->offset(pme[prefix]);
        npeer = peer_resolver.resolve(peer);
        ppeer = Slice(npeer->compressed(), npeer->len());
        pme.iadvance(prefix);
        continue;
      }
      ...
      if (prefix == peer.size() && prefix == pme.size()) {
        offset_t buffer[257], *offsets = &buffer[1];
        memset(buffer, 0, sizeof(buffer));
        offset_t* poff = nme->array();
        int count = 0;
        for (int ch = nme->first(); ch != nme->OUT_OF_RANGE;
             ch = nme->next(), poff++) {
          if (npeer->isset(ch)) {
            auto off = *npeer->offset(ch);
            trie_ptr cpeer = peer_resolver.resolve(off);
            trie_ptr cme = resolve(*poff);
            if (memcpy(cpeer->hash, cme->hash, sizeof(cme->hash))) {
              offsets[ch] = add_inspect(*poff, off);
              count++;
            }
          } else {
            offsets[ch] = -1;
        ...
        trie_ptr n = ndb->alloc(TrieNode::size(prefix, count));
        *parent = ndb->resolve(n);
        n->create(pme, offsets);
        for (offset_t *i = n->array(), end = i + n->count(); i < end, i++) {
          if (*i != -1) inspect_parent(*i, i);

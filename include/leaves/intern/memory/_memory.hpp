...
- _MemManager for size-based page allocation
...
struct _PageContainer : public Traits::PageHeader {
  ...
  std::data(PAGE_SIZES), std::data(PAGE_SIZES) + Traits::PAGE_SIZES_COUNT,
  SIZE);
...
struct _MemManager {
  ...
  static constexpr int assign_slot(uint16_t size) {
    assert(size > 0);
    return binary_search(std::data(PAGE_SIZES), std::data(PAGE_SIZES) + COUNT,
                         size);
  }

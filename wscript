...
def configure(cfg):
    ...
    if cfg.env.CXX_NAME == "gcc":
        ...
        cfg.env.CXXFLAGS_TEST = ["-std=c++17", "-Wall", "-g", "-Warray-bounds=0"]
    ...
    cfg.env.CXXFLAGS_LIB = ["-std=c++17", "-Wall", "-O3", "-Warray-bounds=0"]
...
def build(bld):
    core = "storage.cpp trace.cpp node.cpp page.cpp"
...
def check_lcov():
    try:
        ...
        if e.errno == errno.ENOENT:
            raise ValueError("Error: lcov program not found")
        else:
            raise

...
// New API: takes DBs and root pointers directly
...
void exec_merger(DBDst& dst_db, DBSrc& src_db, MergePolicy& handler) {
  using offset_e = typename DBDst::Traits::offset_e;
  using src_offset_e = typename DBSrc::Traits::offset_e;
  
  // Source root comes from read transaction
  src_offset_e* src_root = &src_db.txn()->root;
  
  uint64_t cursor_id = dst_db.new_cursor_id();
  auto wtxn = dst_db.start_transaction(cursor_id);  // Get write transaction!
  offset_e* dst_root = &wtxn->root;  // Use write transaction's root
  
  _Merger<DBDst, DBSrc, MergePolicy> merger(dst_db, dst_root, src_db, src_root, handler);
  merger.exec();
  dst_db.commit(cursor_id);
}
...
struct OverwritePolicy {
  bool check_overwrite(const std::string& key, const Slice& dst, const Slice& src) {
...
// NodeIter tests disabled - API no longer exists
#if 0
...
BOOST_AUTO_TEST_CASE(test_merge_single_value) {
  ...
  NodeIter iter(src_internal, &src_internal->txn()->root);
